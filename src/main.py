...
holder_right_y = 10
holder_right_z = 0
holder_right_rot_x = 0
holder_right_rot_y = 0
holder_right_rot_z = 0
...
def single_switch_fn():
    ...
    nub_cube = translate(-switch_rim_thickness / 2, 0, 0.0)(
        cube(switch_rim_thickness, nub_len, 4, center=True)
    )
    left_nub = translate(switch_rim_thickness, (outer_width) / 2, 0)(
        hull(nub_cyl, nub_cube)
    )

    right_nub_cube = translate(switch_rim_thickness / 2, 0, 0)(
        cube(switch_rim_thickness, nub_len, 4, center=True)
    )
    right_nub = translate(-switch_rim_thickness + outer_width, (outer_width) / 2, 0)(
        hull(nub_cyl, right_nub_cube)
    )
...
def sa_cap_fn():
    ...
    return colour(220, 163, 163, 1)(
        translate(0, 0, 5 + switch_thickness)(hull(bot, mid, top))
    )
...
def holder_cutout_position():
    ...
    return [base_place[0] + 10, base_place[1] + 10, 6.2]
...
def get_holder_right():
    holder = NativeSCAD(solid.import_stl, "../holder.stl")
    return translate(holder_right_x, holder_right_y, holder_right_z)(
        rotate(a=holder_right_rot_x, v=[1, 0, 0])(
            rotate(a=holder_right_rot_y, v=[0, 1, 0])(
                rotate(a=holder_right_rot_z, v=[0, 0, 1])(holder)
            )
        )
    )
...
def get_holder_left():
    holder = NativeSCAD(solid.import_stl, "../holder.stl")
    return flip_lr()(
        translate(holder_left_x, holder_left_y, holder_left_z)(
            rotate(a=holder_left_rot_x, v=[1, 0, 0])(
                rotate(a=holder_left_rot_y, v=[0, 1, 0])(
                    rotate(a=holder_left_rot_z, v=[0, 0, 1])(holder)
                )
            )
        )
    )
...
def write_test():
    render_to_file(union(right_shell(), get_holder_right()), "things/right.scad")
    render_to_file(union(left_shell(), get_holder_left()), "things/left.scad")
